...
export default function PostCard({ post, currentUser }: PostCardProps) {
  ...
  const hearts = useHeartStore((state) => (postId ? state.hearts[postId] || [] : []));
  const setHearts = useHeartStore((state) => state.setHearts);

  // 초기 hearts 상태 설정
  useEffect(() => {
    if (post?.hearts && postId) {
      setHearts(postId, post.hearts);
    }
  }, [post?.hearts]);
  ...
  const allImages = (Array.isArray(post?.images) ? post.images : []).filter(Boolean);
  ...
  const toggleHeart = async () => {
    ...
    if (hasHeart) {
      ...
      setHearts(postId, hearts.filter((h: any) => h.user_id !== currentUser.id));
    } else {
      await supabase.from("hearts").insert([{ post_id: postId, user_id: currentUser.id }]);
      setHearts(postId, [...hearts, { user_id: currentUser.id }]);
    }
  };
...
const styles = StyleSheet.create({
  ...
  imageScroll: { marginTop: 5 },
  imageItem: { width: 200, height: 200, borderRadius: 8 },
  ...
  icons: { flexDirection: "row", alignItems: "center", gap: 10, marginTop: 5 },
